...
Assert (($types -join ',') -eq 'dotnet-cli,dotnet-durable,dotnet-etl-api,dotnet-mcp,expo,expo-tv') "discovery finds six types, got $($types -join ',')"

$usage = Format-NewProjectUsage -Handlers $handlers
foreach ($t in 'expo', 'expo-tv', 'dotnet-cli', 'dotnet-durable', 'dotnet-etl-api', 'dotnet-mcp') { Assert ($usage -match [regex]::Escape($t)) "usage lists $t" }
