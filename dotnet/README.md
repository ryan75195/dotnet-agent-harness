...
- **16 custom Roslyn analyzers shared by every template** (CI0001–CI0015 and CI0019) that fire at error severity: no comments, method length cap, constructor parameter cap, no tuple returns, no anonymous serialization, no `pragma warning disable`, no chained `null` in arguments, no `Assert.Ignore`, no `IsNotNull`-only assertions, public method count cap, public type per file, test fixture must exist for every public class, etc. The `durable` template adds three more (CI0016–CI0018) that enforce orchestrator determinism, for 19 total.
