...
Each invocation installs the chosen template, scaffolds a smoke project into `$env:TEMP`, builds it, runs all four test projects, and uninstalls. `scaffold-and-build.ps1` doesn't cover `durable` yet — its `-Template` `ValidateSet` only accepts `cli`/`etl-api`/`mcp` — so verify `durable` changes via `dotnet new durable`, a manual build/test, or by pushing and watching CI, which does exercise it. Its integration tests additionally need Azurite running locally (`azurite --silent --inMemoryPersistence --skipApiVersionCheck`, per `setup.ps1` — note `--inMemoryPersistence` and `--location` are mutually exclusive) and Azure Functions Core Tools v4 reachable as a real `func.exe` on `PATH`. That last part is easy to get wrong: the test fixture starts the host via `Process.Start("func")`, which only ever resolves `func.exe`, so `npm install -g azure-functions-core-tools@4` on its own is *not* enough — npm installs `func`/`func.cmd` shims but no `func.exe`. Either install Core Tools via the MSI/winget, or add npm's package directory (`<npm root -g>\azure-functions-core-tools\bin`) to `PATH`, which is what CI does. The unit and architecture tests that the pre-commit hook runs need neither Azurite nor Core Tools.
