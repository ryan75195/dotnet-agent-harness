...
Every change follows this loop. None of these steps are optional â€” hooks enforce each transition.
...
4. **Commit.** `git commit`. `.githooks/pre-commit` runs:
   ...
   - `dotnet build --no-incremental` â€” all analyzers at error severity block
   - `dotnet format --verify-no-changes` â€” style, encoding, line endings
   - `dotnet test` â€” Unit + Architecture projects (Integration not yet wired in)
...
- **No comments.** `NoCommentsAnalyzer` (CI0013) blocks `//`, `/* */`, and `///` at error severity. Extract intent into method names, variable names, or types. If a WHY is genuinely non-obvious (hidden constraint, bug workaround), extract it into a named helper â€” don't write a comment.
...
**Unit vs integration:** a unit test mocks boundaries; an integration test crosses them. Decide based on: *if the mock were wrong, would you ship a bug?* Entry points (endpoints, background services) and persistence (database writes) almost always need integration tests â€” a mock can't verify routing, schema, serialization wire format, or transaction behavior. Everything else: unit test with mocks is usually sufficient.

**What to assert:** assert on the observable outcome the feature promises, not on how it gets there internally. Query â†’ returned data matches expectations. Command â†’ side effect occurred (DB state changed, event published). Transformation â†’ output structure and values are correct. If you're unsure, ask: "what would a user notice if this broke?"
...
- `.githooks/pre-commit` â€” commit-time enforcement
- `.githooks/reference-transaction` â€” branch-creation enforcement
- `.claude/hooks/block-main-branch.sh` â€” edit-time main/master protection
- `.claude/hooks/block-merged-branch.sh` â€” shared merged-branch check
- `.claude/settings.json` â€” Claude Code hook registration
- `Directory.Build.props` â€” `TreatWarningsAsErrors`, analyzer project wire-up
- `.editorconfig` + `tests/.editorconfig` â€” style + severity overrides

## Harness maintenance

This project was scaffolded from the agent-harness template repo.
`.harness.json` records the template and commit it came from â€” never
...
- **Updating the harness.** On any request like "update the harness",
  ...
  plugin). It updates only harness-owned files â€” hooks, lint rules,
  analyzers, CI, this file â€” never your app code, and goes through the
  normal issue â†’ branch â†’ PR lifecycle.
