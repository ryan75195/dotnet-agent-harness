...
public class LayerDependencyTests
{
    ...
    {
        var serverAssembly = Assembly.Load("McpServer.Server");
        var firstPartyRefs = serverAssembly.GetReferencedAssemblies()
            .Select(a => a.Name)
            .Where(name => name?.StartsWith("McpServer", StringComparison.Ordinal) == true)
            .ToList();

        firstPartyRefs.Should().BeEquivalentTo(new[] { "McpServer.Core" },
